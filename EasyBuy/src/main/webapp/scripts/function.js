...
function CheckItem(obj)
{
	...
	switch(obj.name) {
		...
		case "rePassWord":
			if(obj.value == "") {
				msgBox.innerHTML = "纭瀵嗙爜涓嶈兘涓虹┖";
				msgBox.className = "error";
				return false;
			} else if(obj.value != document.getElementById("passWord").value) {
				msgBox.innerHTML = "涓ゆ杈撳叆鐨勫瘑鐮佷笉鐩稿悓";
				msgBox.className = "error";
				return false;
			}
			break;
...
function showChater()
{
	...
	_a.onclick = openRoom;
	_a.appendChild(document.createTextNode("鍦ㄧ嚎鑱婂ぉ"));
	_dd.appendChild(_a);
	_dl.appendChild(_dt);
	_dl.appendChild(_dd);
...
function reloadPrice(id, status)
{
	...
	priceBox.innerHTML = "锟� + price * number.value;
}