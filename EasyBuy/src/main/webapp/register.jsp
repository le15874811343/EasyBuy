...
<div id="register" class="wrap">
	<div class="shadow">
		...
		<div class="box">
			...
			<form id="regForm" method="post" action="/EasyBuy/user/reg.html" >
				<table>
					<tr>
						<td class="field">用户名：</td>
						<td><input class="text" type="text" id= "userName" name="userName"  maxlength="10" onkeyup="value=value.replace(/[\W]/g,'') " onchange="value=value.replace(/[\W]/g,'') " /><span></span></td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td><input class="text" type="password" id="rePassWord" name="rePassWord" /><span></span></td>
					</tr>
					<tr>
						<td class="field">验证码：</td>
						<td><input class="text verycode" maxlength="4" type="text" id="code" name="veryCode"  /><img id="veryCode" src="/EasyBuy/identifyingCode/load.html" /><span></span></td>
					</tr>
					<tr>
						<td></td>
						<td><label class="ui-green"><input type="button" onclick="formSubmit()" name="reg" value="提交注册" /></label></td>
					</tr>
				</table>
			</form>
		</div>
...
function formSubmit(){
	...
	if(rePassWordVal!=passwordVal){
		layer.tips("密码和确认密码不一致！",rePassWord);
		rePassWord.focus();
		return false;
	}
	if(!codeVal){
	...
	if(codeVal){
		...
		$.ajax({
			type : "post",
			url : "/EasyBuy/user/regmatch.html",
			data : {
