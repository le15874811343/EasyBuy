...
<div class="wrap">
		...
		<form  id="buyForm" action="/EasyBuy/order/submit.html" method="post">
			<table id="product">
				<c:if test="${!empty quikPro}">
				    ...
				    <tr id="product_id_1">
					<td class="thumb"><img src="/EasyBuy/${quikPro.epFileName }" onload='if (this.width>140 || this.height>226) if (this.width/this.height>140/226) this.width=50; else this.height=30;' /><a href="/EasyBuy/product/view.html?epId=${quikPro.epId}">${quikPro.epName }</a></td>
					<td class="price" id="price_id_1">
						<span id="${rows }price">${quikPro.epPrice }</span>
						<input type="hidden" value="${quikPro.epPrice }" />
						<input type="hidden" id="${rows }eid" value="${quikPro.epId }">
					</td>
					<td class="number">
						<input type="button" value="+" onclick="changeNumber('+',${rows},${quikPro.epId })"/><input width="10px" id="${rows }number" type="text" name="number" value="${quikPro.userCout }" onkeyup="numberForm(this,${quikPro.epId},${rows })" onchange="numberForm(this)"  /><input type="button" value="-" onclick="changeNumber('-',${rows},${quikPro.epId})" />
					</td>
					<td class="delete"><a href="/EasyBuy/main/welcome.html">取消</a></td>
				</tr>
	...
	<script type="text/javascript">
		...
		for (var i =1;i<product.rows.length-1;i++){
	     ...
	     document.getElementById("money").value=price;
